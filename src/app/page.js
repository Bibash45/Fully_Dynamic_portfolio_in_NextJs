...
export default async function Home() {
  ...
  const projectsSectionData = await extractAllDatas("project");
  
  return <div>
    <ClientHomeView data={homeSectionData} />
    <ClientAboutView  data={aboutSectionData && aboutSectionData.length ? aboutSectionData[0] :[]} />
    <ClientExperienceAndEducationView experienceData={experienceSectionData} educationData={educationSectionData} />
    <ClientProjectView data={projectsSectionData}  />
    <ClientContactView />
  </div>;
}
